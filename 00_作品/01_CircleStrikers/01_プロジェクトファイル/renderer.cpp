...
void Renderer::Draw()
{
	...
	{ //描画開始が成功した場合
		...
		std::vector<Object*> vObjectCamera = Object::Get<Camera>();

		//カメラが付いているオブジェクト分繰り返す
		for (unsigned int i = 0; i < vObjectCamera.size(); i++)
		{
			//カメラを設定する
			vObjectCamera[i]->GetComponent<Camera>()->Set();
		}

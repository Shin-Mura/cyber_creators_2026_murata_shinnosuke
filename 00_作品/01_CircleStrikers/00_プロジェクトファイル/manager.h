...
public:
	...
	void SetSceneMode(Scene::MODE mode);   //シーンの設定処理

	Renderer* GetRenderer() { return m_pRenderer; };   //レンダラーの取得処理
	Texture* GetTexture() { return m_pTexture; };   //テクスチャの取得処理
	XFile* GetXFile() { return m_pXFile; };   //Xファイルの取得処理
	Keyboard* GetKeyboard() { return m_pKeyboard; };   //キーボードの取得処理
	Mouse* GetMouse() { return m_pMouse; };   //マウスの取得処理
	Joypad* GetJoypad() { return m_pJoypad; };   //ジョイパッドの取得処理
	Scene* GetScene() { return m_pScene; };   //シーンの取得処理

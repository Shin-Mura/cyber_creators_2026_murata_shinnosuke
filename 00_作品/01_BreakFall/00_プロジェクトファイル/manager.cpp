...
void Manager::Draw()
{
	...
	{//遷移フラグが立っている場合
		...
		else if (modeNext == Scene::MODE::IS_TUTORIAL)
		{//ランキング画面の場合
			//ランキング画面に変更する
			this->m_pScene->ChangeMode(new SceneModeTutorial(this->m_pScene));
		}
		...
		{//ランキング画面の場合
			//ランキング画面に変更する
			this->m_pScene->ChangeMode(new SceneModeRanking(this->m_pScene));
		}
	}
}